...
def  main():
  ...
  installer = "\installer\Doofuzz Installer.exe"
   
  if demo:
    installer = "\installer\Doofuzz Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Doofuzz manual.pdf" 
  ]
  ...
  ZIPNAME = "Doofuzz-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Doofuzz-v" + FULLVERSIONSTR + "-win-demo.zip"
